...
class Recognizer:
    ...
    def __init__(self, alphabet=None, weights="wow_ocr", build_params=None):
        assert (
            alphabet or weights
        ...
        if weights is not None:
            ...
            self.model.load_weights(
                tools.download_and_verify(
                    url=weights_dict["weights"]["url"],
                    filename=weights_dict["weights"]["filename"],
                    sha256=weights_dict["weights"]["sha256"],
                )
            )
